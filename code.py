...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    aggregate={"mean":np.mean,"max":np.mean,"sum":np.sum,"min":np.minimum,"sub":np.subtract}
    aggregated_value=pd.pivot_table(df,values=agg_col,index=df[date_col].dt.month,aggfunc=         {agg_col:aggregate[agg]})
